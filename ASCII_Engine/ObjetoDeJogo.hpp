...
public:
	...
	void setSprite( const SpriteBase &novoSprite ) // Método adicionado apenas nesse jogo para realizar troca de Sprite
	{
		delete pSprite;  // Libera o sprite antigo para evitar problemas de memória
    	pSprite = novoSprite.copia();  // Cria uma cópia do novo sprite
	}
	
	void ativarObj() {this->active = true;};
	void desativarObj() {this->active = false;};
	bool isActive() {return active;}; // Método adicionado apenas nesse jogo para ver se o objeto está ativo
