...
{
public:
    Fase2(std::string name, const Sprite &bkg, EstadoJogador* estado) : Fase(name,bkg), estado(estado) {}
    Fase2(std::string name, const SpriteAnimado &bkg, EstadoJogador* estado) : Fase(name,bkg), estado(estado) {}
    virtual ~Fase2(){}
    ...
    void init()
    {
        ...
        Enemy* guarda1 = new Enemy(ObjetoDeJogo("Guarda6",SpriteAnimado("rsc/guardaArmadoE.anm",3), 39, 58), 'a');
        guardas.push_back(guarda1);
        objs.push_back(guardas.back());

        Enemy* guarda2 = new Enemy(ObjetoDeJogo("Guarda7",SpriteAnimado("rsc/guardaArmadoE.anm",3), 32, 108), 'a');
        guardas.push_back(guarda2);
        objs.push_back(guardas.back());

        Enemy* guarda3 = new Enemy(ObjetoDeJogo("Guarda8",SpriteAnimado("rsc/guardaArmadoE.anm",3), 47, 158), 'a');
        ...
        objs.push_back(new ObjetoDeJogo("PFD", Sprite("rsc/paredeFundo.img"), 6, 216));
        colisoes.push_back(objs.back());
    }
    ...
    void verificarAtaques()
    {
        for (auto guarda : guardas) {
            ...
            if (guarda->isActive() && guarda->podeAtirar()) {
                balas.push_back(new Bala(ObjetoDeJogo("Bala", Sprite("rsc/bala.img"), guarda->getPosL() + 3, guarda->getPosC() - 5), guarda->getDirecao(), 5));
                objs.push_back(balas.back());
            }
        }
    }
